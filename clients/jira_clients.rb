  ...
  class JiraClient
    ...
    def self.build_from_config!(config)
      ...
      if email.nil? || email.strip.empty?
        log_error "Le paramètre de configuration 'jira.email' est obligatoire"
        exit 1
      end

      if token.nil? || token.strip.empty?
        log_error "Le paramètre de configuration 'jira.token' est obligatoire"
        exit 1
      end
    ...
    def request(method, endpoint, body = nil)
      ...
      unless response.is_a?(Net::HTTPSuccess)
        ...
        raise "Erreur API (#{response.code}): #{error_msg}"
      end
